...
class TestDepthMapSurfaceNormalCalculator(unittest.TestCase):
    ...
    def test_depth_map_to_point_cloud_shape(self):
        depth_map = np.ones((1, 480, 640)) # Batch size 1
    ...
    def test_flat_surface_normal_direction(self):
        ...
        self.assertAlmostEqual(n_center[0], 0.0, places=5)
        self.assertAlmostEqual(n_center[1], 0.0, places=5)
        # It could be negative depending on cross product order, but the magnitude is 1
        self.assertAlmostEqual(abs(n_center[2]), 1.0, places=5)

    def test_invalid_intrinsics_and_inputs(self):
        # Invalid intrinsics size
        with self.assertRaises(ValueError):
            DepthMapSurfaceNormalCalculator(camera_intrinsics=(500.0, 500.0, 320.0))
        
        # Invalid input type
        with self.assertRaises(TypeError):
            self.calculator.calculate_surface_normals("not a tensor")
            
        # Invalid dimensions
        with self.assertRaises(ValueError):
            self.calculator.calculate_surface_normals(np.ones((5,)))
